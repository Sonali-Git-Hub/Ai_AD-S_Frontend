...
`;
  ...
  const getItemSize = (index) => {
    ...
    if (hasSpacerRow && index === messages.length + (isTypingIndicatorActive ? 1 : 0)) {
      return window.innerWidth < 768 ? 256 : 288;
    }
    ...
    let size = 80; // Base height including avatar, vertical gap, and margins

    if (msg.content || msg.text) {
      const text = msg.content || msg.text;
      const charsPerLine = window.innerWidth < 640 ? 30 : 65;
      const lines = Math.ceil(text.length / charsPerLine);
      size += lines * 22; // approx 22px line height

      // Add extra height for formatting markers
      const tableMatches = text.match(/\|/g);
      if (tableMatches) {
        size += (tableMatches.length / 4) * 45;
      }

      const codeMatches = text.match(/\`\`\`/g);
      if (codeMatches) {
        size += (codeMatches.length / 2) * 200;
      }
      
      const newlineMatches = text.match(/\n/g);
      if (newlineMatches) {
        size += newlineMatches.length * 22;
      }
    }
    ...
    if (msg.attachments && msg.attachments.length > 0) {
      size += msg.attachments.length * 80;
    }
    ...
    if (msg.role === 'model' && msg.isRealTime && msg.sources && msg.sources.length > 0) {
      size += 120;
    }

    if (msg.conversion && msg.conversion.mimeType?.startsWith('audio/')) {
      size += 180;
    }

    if (index === messages.length - 1 && suggestions && suggestions.length > 0) {
      size += 60;
    }
  ...
  }, [messages, suggestions, expandedMessages, isTypingIndicatorActive]);
    ...
    <div className="flex w-full bg-transparent relative overflow-hidden aisa-scalable-text h-full">
      ...
      >
        ...
        <ChatWelcome
          ...
          activeLegalToolkit={activeLegalToolkit}
          selectedLegalTool={selectedLegalTool}
        ...
        <ChatInput
          ...
          setIsAudioConvertMode={setIsAudioConvertMode}
          setIsDocumentConvert={setIsDocumentConvert}
          ...
          setIsFileAnalysis={setIsFileAnalysis}
          ...
          setIsSendHovered={setIsSendHovered}
          toast={toast}
          navigate={navigate}
          ...
          handleDocToVoiceSelect={handleDocToVoiceSelect}
